...
if ! ssh-keyscan -p "$INPUT_REMOTE_DOCKER_PORT" "$SSH_HOST" >> ~/.ssh/known_hosts 2>/dev/null; then
  echo "Warning: ssh-keyscan failed for $SSH_HOST:$INPUT_REMOTE_DOCKER_PORT"
fi
...
if ! [ -z "${INPUT_COPY_STACK_FILE+x}" ] && [ "$INPUT_COPY_STACK_FILE" = 'true' ] ; then
  ...
  # Validate local stack file exists before copying
  ...
  # Use find + sort for safe cleanup (handles filenames with spaces)
