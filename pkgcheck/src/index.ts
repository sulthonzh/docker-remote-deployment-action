...
function checkPackageJsonExists(dir: string): CheckResult {
...
function checkName(pkg: PkgJson | null): CheckResult {
  if (!pkg || !pkg.name) {
...
function checkVersion(pkg: PkgJson | null): CheckResult {
  if (!pkg || !pkg.version) {
...
function checkDescription(pkg: PkgJson | null): CheckResult {
...
function checkLicense(pkg: PkgJson | null): CheckResult {
  if (!pkg || !pkg.license) {
...
function checkRepository(pkg: PkgJson | null): CheckResult {
...
function checkKeywords(pkg: PkgJson | null): CheckResult {
...
function checkMainEntry(dir: string, pkg: PkgJson | null): CheckResult {
  if (!pkg) return { name: "entry point", passed: false, message: "no package.json", severity: "error" };
  if (pkg.bin && !pkg.main) {
...
function checkBin(dir: string, pkg: PkgJson | null): CheckResult {
  if (!pkg || !pkg.bin) return { name: "bin", passed: true, message: "no bin — not a CLI", severity: "info" };
  const bins: Record<string, string> = typeof pkg.bin === "string" ? { [pkg.name || "bin"]: pkg.bin } : pkg.bin;
  const missing: string[] = [];
  for (const [, target] of Object.entries(bins)) {
    if (!existsSync(join(dir, target))) missing.push(target);
  }
  if (missing.length > 0) {
...
function checkBuildScript(pkg: PkgJson | null): CheckResult {
  if (!pkg || !pkg.scripts) {
  ...
  if (pkg.main && extname(pkg.main) === ".ts") {
...
function checkPrepublishOnly(pkg: PkgJson | null): CheckResult {
  if (!pkg || !pkg.scripts) return { name: "prepublishOnly", passed: true, message: "no scripts", severity: "info" };
...
function checkFilesField(pkg: PkgJson | null): CheckResult {
  if (!pkg || !pkg.files) {
...
function checkPrivate(pkg: PkgJson | null): CheckResult {
  if (!pkg) return { name: "private", passed: false, message: "no package.json", severity: "error" };
  if (pkg.private === true) {
...
function checkDeps(pkg: PkgJson | null): CheckResult {
  if (!pkg) return { name: "dependencies", passed: false, message: "no package.json", severity: "error" };
  const all = { ...pkg.dependencies, ...pkg.peerDependencies };
  const deps = Object.entries(all);
  if (deps.length === 0) return { name: "dependencies", passed: true, message: "zero deps 👌", severity: "info" };
  
  const issues: string[] = [];
  for (const [name, version] of deps) {
    if (!version) { issues.push(`${name}: no version`); continue; }
...
function checkNpmignore(dir: string, pkg: PkgJson | null): CheckResult {
...
function checkDuplicateDeps(pkg: PkgJson | null): CheckResult {
  if (!pkg) return { name: "duplicate deps", passed: true, message: "n/a", severity: "info" };
...
function checkTypes(dir: string, pkg: PkgJson | null): CheckResult {
  if (!pkg) return { name: "types", passed: true, message: "n/a", severity: "info" };
  const hasDist = existsSync(join(dir, "dist"));
  if (!hasDist && !pkg.main?.includes(".ts")) return { name: "types", passed: true, message: "not a TS project", severity: "info" };
  if (pkg.types || pkg.typings) {
    return { name: "types", passed: true, message: pkg.types || pkg.typings || "", severity: "info" };
  }
  if (hasDist && existsSync(join(dir, "dist", "index.d.ts"))) {
...
function checkEngineNode(pkg: PkgJson | null): CheckResult {
...
function checkAuthor(pkg: PkgJson | null): CheckResult {
  if (!pkg || !pkg.author) {
...
function checkHomepage(pkg: PkgJson | null): CheckResult {
  if (!pkg || !pkg.homepage) {
...
function checkBugsUrl(pkg: PkgJson | null): CheckResult {
  if (!pkg || !pkg.bugs) {
...
function checkDistFiles(dir: string, pkg: PkgJson | null): CheckResult {
  ...
  const distDir = "dist";
  if (!existsSync(join(dir, distDir))) {
  ...
  for (const file of files) {
    if (!existsSync(join(dir, distDir, file))) {
      missing.push(file);
    }
  }
...
function checkVulnerabilities(pkg: PkgJson | null): CheckResult {
  if (!pkg) return { name: "vulnerabilities", passed: true, message: "n/a", severity: "info" };
...
export function formatResults(results: CheckResult[], options: CheckOptions = {}): string {
  const lines: string[] = [];
